...
then
	...
	case $GPU in
		...
		nvidia)
			...
			echo "pacman --noconfirm -Sy xf86-video-vesa nvidia lib32-nvidia-utils nvidia-utils nvidia-settings nvidia-dkms vulkan-icd-loader lib32-vulkan-icd-loader optimus-manager optimus-manager-qt  egl-wayland  || exit 1" | ARCH && DONE || ERROR ;;
  		intel)
...
yay -S --noconfirm sddm plasma-meta plasma-wayland-session kde-system-meta kde-utilities-meta dolphin-plugins packagekit-qt5 print-manager cups system-config-printer && \
